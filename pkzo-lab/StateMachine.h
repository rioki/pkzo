...
namespace rex
{
    ...
    public:
        ...
        //!
        //! This overload queues a transition without an enter payload.
        //!
        ...
        //! Schedule a state transition with an optional enter payload.
        //!
        //! The payload is stored as std::any and passed to the target state's
        //! on_enter callback when the transition is applied. Calling queue_state
        //! again before tick() replaces both the queued state and payload.
        //!
        //! @param new_state The state to transition to on the next tick.
        //! @param new_value Value passed to on_enter as std::any.
        template <typename T>
        void queue_state(StateEnum new_state, T&& new_value)
        {
            next_value = std::any(std::forward<T>(new_value));
            next_state = new_state;
        }
        ...
        //! first, including its enter and exit callbacks. If a payload was
        //! queued, it is passed to the new state's on_enter callback as
        //! std::any. The tick callback of the resulting state is then called
        ...
        void tick(float dt)
        {
            ...
            while (next_state)
            {
                auto state = std::exchange(next_state, std::nullopt);
                change_state(*state);
            }
        ...
        //!               Receives the queued enter payload as std::any.
        //! @throws std::invalid_argument if func is null.
        //! @throws std::logic_error if a callback is already registered for this state.
        void on_enter(StateEnum state, const std::function<void (const std::any&)>& func)
        {
            if (func == nullptr)
            {
        ...
        //! Register a callback to be invoked when entering a state.
        //!
        //! Convenience overload for enter callbacks that do not require the
        //! queued payload value.
        //!
        //! @param state  The state to register the callback for.
        //! @param func   Callback invoked when the machine enters this state.
        //! @throws std::invalid_argument if func is null.
        ...
        {
            on_enter(state, [=] (const std::any&) { func(); });
        }
    ...
    private:
        ...
        std::map<StateEnum, std::function<void (const std::any&)>>      enter_functions;
