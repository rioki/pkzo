...
namespace internal
{
    ...
    template<unsigned int N, typename T, glm::qualifier Q>
    testing::AssertionResult CmpHelperGlmNE(const char* lhs_expression, const char* rhs_expression, const glm::vec<N, T, Q>& lhs_value, const glm::vec<N, T, Q>& rhs_value)
    {
        if (!glm::all(glm::equal(lhs_value, rhs_value))) {
            return testing::AssertionSuccess();
        }
        ...
        ::std::stringstream rhs_ss;
        rhs_ss << rhs_value;
        return testing::internal::CmpHelperOpFailure(lhs_expression,
                         rhs_expression,
                         testing::internal::StringStreamToString(&lhs_ss),
                         testing::internal::StringStreamToString(&rhs_ss),
                         "!=");
    ...
    template<unsigned int N, unsigned int M, typename T, glm::qualifier Q>
    testing::AssertionResult CmpHelperGlmNe(const char* lhs_expression, const char* rhs_expression, const glm::mat<N, M, T, Q>& lhs_value, const glm::mat<N, M, T, Q>& rhs_value)
    {
        bool pass = true;
        for (auto i = 0u; i < N; i++)
        {
...
#define EXPECT_GLM_NE(val1, val2) EXPECT_PRED_FORMAT2(::internal::CmpHelperGlmNE, val1, val2)
#define ASSERT_GLM_NE(val1, val2) ASSERT_PRED_FORMAT2(::internal::CmpHelperGlmNE, val1, val2)
